...
- 场景骨架：根据内容切换走廊/站台、电梯间、卧室、教室、宫殿大厅、海边、糖果屋等基础空间。
- 场景规划：先把梦境分镜转换为 `scenePlan` JSON，再由 Three.js 按规划搭建空间、道具和触发点。
- 风格画像：输入后可用色盘自选主色调，也可叠加情绪关键词，影响润色文本、`scenePlan` 的 mood/palette，以及 3D 场景的背景、雾、灯光和材质。
- 梦境变换：识别“突然变成、变换、镜头一转”等荒诞转场，生成 `morph` 连接；回放中可点击“变换”或按 `T` 跳切到下一段梦境。
